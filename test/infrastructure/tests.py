...
class InvokeTest(Test):
	...
	def invoke(self, gEx):
		...
		execCmd = [os.path.abspath(gEx), self.phase, self.srcfile]
		try:
...
class PositiveTest(InvokeTest):
	...
	def __init__(self, base, src, res, ph):
		self.basedir = base
		self.srcfile = src
		self.phase = ph
		self.result = res
	...
	def checkOutput(self, success, output):
		if not success:
			print("\nThe test "+os.path.join(self.basedir, self.srcfile)+" failed, but it should not. ");
			print("Output: "+output);
			return False;
		if "" == self.result:
			return True
...
class NegativeTest(InvokeTest):
	...
	def checkOutput(self, success, output):
		if success:
			print("\nThe test "+self.srcfile+" is negative, but the compiler did not indicate an error.");
			return False;
		if "" == self.result:
			print("\nNo expected output!")
			return False
		# check whether the string in result is the first error
		errPos = output.find("error:")
		findPos = output.find(self.result)
		if -1 == findPos:
			return False
		# check whether the error I looked for is the first one
		return errPos > findPos
