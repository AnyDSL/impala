...
def analyze_returncode(returncode):
    if returncode < 0:
        return (False, 'was terminated by signal {}'.format(-returncode))
    if returncode > 0:
        return (False, "didn't run successfully with exit code {}".format(returncode))
    return (True, '')
...
def run_tests():
    def worker(testsuit):
        def run_test():
            ...
            output = '{}{} {}{}'.format('broken ' if broken else '', 'passed' if test.result == PASSED else 'FAILED', test_path, output)
            print(output)
