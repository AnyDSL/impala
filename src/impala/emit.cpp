                ...
                if (auto fn_decl = path->value_decl()->isa<FnDecl>()) {
                    if (fn_decl->is_extern() && fn_decl->abi() == "\"thorin\"") {
                        ...
                        if (name == "bitcast") {
                            ...
                            cont->set_intrinsic();
                            dst = cont;
                        } else if (name == "comm_type") {
                            auto input = cg.convert(arg(0)->type());
                            //mpi datatype
                            auto fn_type = cg.world.fn_type({
                                cg.world.mem_type(), input,
                                cg.world.fn_type({ cg.world.mem_type(), cg.world.ptr_type(cg.world.type_qs32(1)) }) });
                            auto cont = cg.world.continuation(fn_type, {location(), "comm_type"});
                            cont->set_intrinsic();
                            dst = cont;
                        }
                    }
