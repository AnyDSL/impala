...
public:
    ...
    Dbg dbg(const Decl* decl) { return {decl->loc(), world.sym(decl->symbol().str())}; }
    ...
    std::pair<Lam*, const Def*> call(const Def* callee, Defs args, const mim::Def* ret_type) {
        if (ret_type == nullptr) {
    ...
    Lam* create_lam(const LocalDecl* decl) {
        auto result = world.mut_lam(convert(decl->type())->as<mim::Pi>())->set(dbg(decl));
    ...
    const mim::Def* rev_diff(const mim::Def* /*primal*/) { return nullptr; /*world.op_rev_diff(primal);*/ }
    ...
    const mim::Def*& thorin_type(const Type* type) { return impala2thorin_[type]; }
    const mim::Sigma*& thorin_struct_type(const StructType* type) { return struct_type_impala2thorin_[type]; }
    const mim::Sigma*& thorin_enum_type(const EnumType* type) { return enum_type_impala2thorin_[type]; }
...
void LocalDecl::emit(CodeGen& cg, const Def* init) const {
    ...
    if (is_mut()) {
        ...
        cg.cur_mem = cg.world.call<mem::store>(Defs{cg.cur_mem, def_, init})->set(loc());
    } else {
        def_ = init;
    }
}
...
const Def* CharExpr::remit(CodeGen& cg) const { return cg.world.lit_idx<u8>(value())->set(loc()); }

const Def* StrExpr::remit(CodeGen& cg) const {
    Array<const Def*> args(values_.size());
    for (size_t i = 0, e = args.size(); i != e; ++i) args[i] = cg.world.lit_idx<u8>(values_[i])->set(loc());
...
const Def* CastExpr::remit(CodeGen& cg) const {
    ...
    if (src_type->isa<PtrType>() || dst_type->isa<PtrType>()) {
        ...
        else if (is_int(dst_type) || is_bool(dst_type))
...
const Def* RValueExpr::remit(CodeGen& cg) const {
    if (src()->type()->isa<RefType>()) return cg.load(lemit(cg))->set(loc());
    return src()->remit(cg);
}
...
const Def* PrefixExpr::remit(CodeGen& cg) const {
    switch (tag()) {
        ...
        case DEC: {
            ...
            if (is_int(type()))
                val = cg.world
                          .call(tag() == INC ? core::wrap ::add : core::wrap ::sub, type2wmode(type()), Defs{val, one})
                          ->set(loc());
            else
                val = cg.world
                          .call(tag() == INC ? math::arith::add : math::arith::sub, math::Mode::none, Defs{val, one})
                          ->set(loc());
...
const Def* PostfixExpr::remit(CodeGen& cg) const {
    ...
    if (is_int(type()))
        val = cg.world.call(tag() == INC ? core::wrap::add : core::wrap::sub, type2wmode(type()), Defs{res, one})
                  ->set(loc());
    else
        val = cg.world.call(tag() == INC ? math::arith::add : math::arith::sub, math::Mode::none, Defs{res, one})
                  ->set(loc());
    cg.store(var, val)->set(loc());
    return res;
}
...
const Def* MapExpr::remit(CodeGen& cg) const {
    ...
    if (auto cn = ltype->isa<FnType>()) {
        ...
        // Handle primops here
        if (auto type_expr = lhs()->isa<TypeAppExpr>()) { // Bitcast, sizeof and select are all
                                                          // polymorphic
            ...
            if (auto path = callee->isa<PathExpr>()) {
                if (auto fn_decl = path->value_decl()->isa<FnDecl>()) {
                    if (fn_decl->is_extern() && fn_decl->abi() == "\"thorin\"") {
                        ...
                        if (name == "bitcast") {
                            return w.call<core::bitcast>(cg.convert(type_expr->type_arg(0)), arg(0)->remit(cg))
                                ->set(loc());
                        } else if (name == "select") {
                            return w.extract(w.tuple({arg(2)->remit(cg), arg(1)->remit(cg)}), arg(0)->remit(cg))
                                ->set(loc());
                        } else if (name == "insert") {
                            return core::insert_unsafe(arg(0)->remit(cg), arg(1)->remit(cg), arg(2)->remit(cg))
                                ->set(loc());
                        } else if (name == "alignof") {
                            return w
                                .call<core::bitcast>(w.type_int(32),
                                                     w.call(core::trait::align, cg.convert(type_expr->type_arg(0))))
                                ->set(loc());
                        } else if (name == "sizeof") {
                            return w
                                .call<core::bitcast>(w.type_int(32),
                                                     w.call(core::trait::size, cg.convert(type_expr->type_arg(0))))
                                ->set(loc());
                        } else if (name == "undef") {
...
const Def* IfExpr::remit(CodeGen& cg) const {
    ...
    auto if_then   = cg.world.mut_lam(jump_type)->set(then_expr()->loc().begin, "if_then");
    auto if_else   = cg.world.mut_lam(jump_type)->set(else_expr()->loc().begin, "if_else");
    auto if_join   = thorin_type ? cg.basicblock(thorin_type)->set(loc().finis, "if_join")
                                 : nullptr; // TODO rewrite with bottom type

    cond()->emit_branch(cg, if_then, if_else);

    cg.enter(if_then);
    if (auto tdef = then_expr()->remit(cg)) cg.cur_bb->app(false, if_join, {cg.cur_mem, tdef})->set(loc().finis);

    cg.enter(if_else);
    if (auto fdef = else_expr()->remit(cg)) cg.cur_bb->app(false, if_join, {cg.cur_mem, fdef})->set(loc().finis);
...
const Def* WhileExpr::remit(CodeGen& cg) const {
    auto head_bb = cg.world.mut_lam(cg.world.cn(cg.world.annex<mem::M>()))->set(loc().begin, "while_head");
    head_bb->var(0)->set("mem");

    auto jump_type = cg.world.cn(cg.world.annex<mem::M>());
    auto body_bb   = cg.world.mut_lam(jump_type)->set(body()->loc().begin, "while_body");
    auto exit_bb   = cg.world.mut_lam(jump_type)->set(body()->loc().finis, "while_exit");
...
void IdPtrn::emit(CodeGen& cg, const mim::Def* init) const { local()->emit(cg, init); }

const mim::Def* IdPtrn::emit_cond(CodeGen& cg, const mim::Def*) const { return cg.world.lit_tt(); }

void EnumPtrn::emit(CodeGen& cg, const mim::Def* init) const {
    if (num_args() == 0) return;
    auto variant_type = path()->decl()->as<OptionDecl>()->variant_type(cg);
    auto variant      = cg.world.call<core::bitcast>(variant_type, cg.world.extract(init, num_args(), 1))->set(loc());
    for (size_t i = 0, e = num_args(); i != e; ++i)
        arg(i)->emit(cg, num_args() == 1 ? variant : cg.world.extract(variant, num_args(), i)->set(loc()));
}
...
const mim::Def* EnumPtrn::emit_cond(CodeGen& cg, const mim::Def* init) const {
    ...
    auto cond   = cg.world.call(core::icmp::e, Defs{init_0, cg.world.lit_idx(u32(index))})->set(loc());
    if (num_args() > 0) {
        auto variant_type = path()->decl()->as<OptionDecl>()->variant_type(cg);
        auto variant
            = cg.world.call<core::bitcast>(variant_type, cg.world.extract(init, num_args(), 1)->set(loc()))->set(loc());
        for (size_t i = 0, e = num_args(); i != e; ++i) {
            if (!arg(i)->is_refutable()) continue;
            auto arg_cond = arg(i)->emit_cond(
                cg, num_args() == 1 ? variant : cg.world.extract(variant, num_args(), i)->set(loc()));
            cond = cg.world.call(core::bit2::and_, 0, Defs{cond, arg_cond})->set(loc());
        }
    }
    return cond;
}
...
void TuplePtrn::emit(CodeGen& cg, const mim::Def* init) const {
    for (size_t i = 0, e = num_elems(); i != e; ++i) elem(i)->emit(cg, cg.world.extract(init, e, i)->set(loc()));
}
...
const mim::Def* TuplePtrn::emit_cond(CodeGen& cg, const mim::Def* init) const {
    ...
    for (size_t i = 0, e = num_elems(); i != e; ++i) {
        ...
        auto next = elem(i)->emit_cond(cg, cg.world.extract(init, num_elems(), i)->set(loc()));
...
const mim::Def* LiteralPtrn::emit_cond(CodeGen& cg, const mim::Def* init) const {
...
void LetStmt::emit(CodeGen& cg) const {
    ptrn()->emit(cg, init() ? init()->remit(cg) : cg.world.bot(cg.convert(ptrn()->type()))->set(ptrn()->loc()));
}
