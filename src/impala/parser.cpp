...
public:
    ...
    void nibble_comma_list(Array<TokenTag> delimiters, std::function<void()> f) {
        auto is_delimiter = [&]() {
            for (auto delimiter : delimiters)
                if (lookahead() == delimiter) return true;
            return false;
        };

        if (!is_delimiter()) {
            do { f(); } while (accept(Token::COMMA) && !is_delimiter());
        }
    }
    ...
    bool param_list() {
        return lookahead(0) == Token::IN || lookahead(0) == Token::MUT || lookahead(1) == Token::COLON
            || lookahead(1) == Token::COMMA || lookahead(1) == Token::IN;
    }
...
void parse(Items& items, std::istream& is, const std::filesystem::path* path) {
    ...
    if (parser.lookahead() != Token::Eof) parser.error("module item", "module contents");
}
...
bool Parser::accept(TokenTag type) {
    if (type != lookahead()) return false;
    lex();
    return true;
}
...
uint64_t Parser::parse_integer(const char* what) {
    switch (lookahead()) {
        ...
        default: error("integer literal", what); return 0;
    }
}
...
const Path::Elem* Parser::parse_path_elem() { return new Path::Elem(try_identifier("path")); }

const Path* Parser::parse_path() {
    auto tracker   = track();
    bool is_global = accept(Token::DOUBLE_COLON);
    Path::Elems elems;
    do { elems.emplace_back(parse_path_elem()); } while (accept(Token::DOUBLE_COLON));
...
const ASTTypeParam* Parser::parse_ast_type_param() {
    ...
    if (accept(Token::COLON)) {
        do { bounds.emplace_back(parse_type()); } while (accept(Token::ADD));
    }
...
const Param* Parser::parse_param(int /*i*/, bool lambda) {
    ...
    if (tok == Token::ID)
        ...
        switch (tok) {
            ...
            default: identifier = new Identifier(tok.loc(), "<error>"); error("identifier", "parameter");
        }
    }

    if (accept(Token::COLON)) {
        if (type) error("identifier", "parameter", tok);
        ast_type = parse_type();
    } else if (lambda) {
        if (type) error("identifier", "parameter", tok);
    } else {
    ...
    if (identifier == nullptr) identifier = create<Identifier>("_");
...
const Item* Parser::parse_extern_block_or_fn_decl(Tracker tracker, Visibility vis) {
    ...
    Symbol abi;
    if (lookahead() == Token::LIT_str) abi = lex().symbol();
...
const FnDecl* Parser::parse_fn_decl(BodyMode mode, Tracker tracker, Visibility vis, bool is_extern, Symbol abi) {
    ...
    if (auto ret_param = parse_return_param()) params.emplace_back(ret_param);
    ...
    switch (mode) {
        ...
        case BodyMode::Optional:
            if (!accept(Token::SEMICOLON)) body = try_block_expr("body of function");
            break;
    }

    return new FnDecl(tracker, vis, is_extern, abi, filter, export_name, identifier, std::move(ast_type_params),
...
void Parser::parse_items(Items& items) {
    while (true) {
        switch (lookahead()) {
            case VISIBILITY:
            case ITEM: items.emplace_back(parse_item()); continue;
...
const TraitDecl* Parser::parse_trait_decl(Tracker tracker, Visibility vis) {
    ...
    if (accept(Token::COLON))
        parse_comma_list("trait declaration", Token::L_BRACE, [&] { super_traits.emplace_back(parse_ast_type_app()); });
    ...
    return new TraitDecl(tracker, vis, identifier, std::move(ast_type_params), std::move(super_traits),
                         std::move(methods));
}
...
const FnASTType* Parser::parse_fn_type() {
    ...
    bool unused;
    if (auto ret_type = parse_return_type(unused, /*mandatory*/ true)) ast_type_args.emplace_back(ret_type);
...
const PtrASTType* Parser::parse_ptr_type() {
    ...
    if (accept(Token::ANDAND)) {
        ...
        return new PtrASTType(tracker, PtrASTType::Borrowed, 0,
...
const Expr* Parser::parse_expr(Prec prec) {
    ...
    while (true) {
        ...
        if (lookahead().is_infix()) {
            if (prec > PrecTable::infix_l(lookahead())) break;

            lhs = parse_infix_expr(tracker, lhs);
        } else if (lookahead().is_postfix()) {
            if (prec > Prec::Unary) break;
...
const Expr* Parser::parse_prefix_expr() {
    if (lookahead() == Token::OR || lookahead() == Token::OROR || lookahead() == Token::RUN) return parse_fn_expr();
...
const Expr* Parser::parse_infix_expr(Tracker tracker, const Expr* lhs) {
    auto tag = lex().tag();
    if (tag == Token::AS) return new ExplicitCastExpr(tracker, lhs, parse_type());
...
#include "impala/tokenlist.h"
        ...
        case Token::ID: {
            ...
            if (lookahead(0) == Token::L_BRACE
                && ((lookahead(1) == Token::ID && lookahead(2) == Token::COLON) || lookahead(1) == Token::R_BRACE)) {
                eat(Token::L_BRACE);
    ...
    if (*p++ == '\\') {
        ...
        case '\\': value = '\\'; break;
            ...
            default:
                ...
                // chars
                impala::error(lookahead().loc(), "expected valid escape sequence, got '\\{}' while parsing {}",
                              *(p - 1), lookahead());
        }
    } else
        value = *(p - 1);
...
const StrExpr* Parser::parse_str_expr() {
    ...
    do {
        ...
        assert(*p == '"');
        ++p;
        while (*p != '"') values.emplace_back(char_value(p));
        assert(p[1] == '\0');
        lex();
...
const FnExpr* Parser::parse_fn_expr(bool nested) {
    ...
    if (accept(Token::OR) || nested) {
        ...
        int i = 0;
        nibble_comma_list({Token::OR, Token::OROR}, [&] { params.emplace_back(parse_param(i++, true)); });
...
const IfExpr* Parser::parse_if_expr() {
    ...
    if (else_expr == nullptr) else_expr = create<BlockExpr>();
...
const BlockExpr* Parser::try_block_expr(const std::string& context) {
    switch (lookahead()) {
        case Token::L_BRACE: return parse_block_expr();
        default: error("block expression", context); return create<BlockExpr>();
    }
}
...
const TuplePtrn* Parser::parse_tuple_ptrn() {
    ...
    eat(Token::L_PAREN);
    Ptrns elems;
    parse_comma_list("closing parenthesis of tuple pattern", Token::R_PAREN, [&] { elems.emplace_back(parse_ptrn()); });
...
const CharPtrn* Parser::parse_char_ptrn() { return new CharPtrn(parse_char_expr()); }
