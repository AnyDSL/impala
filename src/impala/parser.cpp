...
#define VISIBILITY Token::PRIV : case Token::PUB

#define ITEM                                                                   \
  Token::ENUM : case Token::EXTERN:                                            \
  case Token::FN:                                                              \
  case Token::IMPL:                                                            \
  case Token::MOD:                                                             \
  case Token::STATIC:                                                          \
...
#define EXPR                                                                   \
  Token::LIT_i8 : case Token::LIT_i16:                                         \
  case Token::LIT_i32:                                                         \
  case Token::LIT_i64:                                                         \
  case Token::LIT_u8:                                                          \
  case Token::LIT_u16:                                                         \
...
#define STMT                                                                   \
  Token::LET : case Token::ASM:                                                \
  case ITEM:                                                                   \
  case EXPR

#define TYPE                                                                   \
  Token::FN : case Token::L_PAREN:                                             \
  case Token::ID:                                                              \
  case Token::L_BRACKET:                                                       \
  case Token::TYPE_i8:                                                         \
  case Token::TYPE_i16:                                                        \
...
public:
  ...
  const Token &lookahead(size_t i = 0) const {
    assert(i < 3);
    return lookahead_[i];
  }
...
#else
  Token eat(TokenTag tag) {
    assert(tag == lookahead() && "internal parser error");
    return lex();
  }
#endif
  ...
  class Tracker {
  public:
    Tracker(Parser &parser, const Loc &loc) : parser_(parser), loc_(loc) {}
  ...
  template <class T, class... Args> const T *create(Args &&...args) {
    return new T(prev_loc(), std::forward<Args>(args)...);
  }

  /**
   * Parses a list of comma-separated items till one of the @p delimiters have
   * been found. The ending delimiter will @em not be eaten up by this method.
  ...
  /// Like @p nibble_comma_list but there is only one @p delimiter which @em
  /// will be eaten up by this method.
  void parse_comma_list(const char *context, TokenTag delimiter,
                        std::function<void()> f) {
  ...
  bool param_list() {
    return lookahead(0) == Token::IN || lookahead(0) == Token::MUT ||
           lookahead(1) == Token::COLON || lookahead(1) == Token::COMMA ||
           lookahead(1) == Token::IN;
  }
  ...
  const FnDecl *parse_fn_decl(BodyMode, Tracker, Visibility, bool is_extern,
                              Symbol abi);
...
private:
  /// Consume next Token in input stream, fill look-ahead buffer, return
  /// consumed Token.
  Token lex();
...
ASTTypeParams Parser::parse_ast_type_params() {
  ...
  if (accept(Token::L_BRACKET))
    parse_comma_list("type parameter list", Token::R_BRACKET, [&] {
      ast_type_params.emplace_back(parse_ast_type_param());
    });
...
Params Parser::parse_param_list(TokenTag delimiter, bool lambda) {
  Params params;
  int i = 0;
  parse_comma_list("parameter list", delimiter,
                   [&] { params.emplace_back(parse_param(i++, lambda)); });
  return params;
}
...
const Item *Parser::parse_item() {
  ...
  switch (lookahead()) {
  case Token::ENUM:
    return parse_enum_decl(tracker, vis);
  case Token::EXTERN:
    return parse_extern_block_or_fn_decl(tracker, vis);
  case Token::FN:
    return parse_fn_decl(BodyMode::Mandatory, tracker, vis, /*extern*/ false,
                         /*abi*/ "");
  case Token::IMPL:
    return parse_impl(tracker, vis);
  case Token::MOD:
    return parse_module_or_module_decl(tracker, vis);
  case Token::STATIC:
    return parse_static_item(tracker, vis);
  case Token::STRUCT:
    return parse_struct_decl(tracker, vis);
  case Token::TRAIT:
    return parse_trait_decl(tracker, vis);
  case Token::TYPEDEF:
    return parse_typedef(tracker, vis);
  default:
    fe::unreachable();
  }
}
...
const EnumDecl *Parser::parse_enum_decl(Tracker tracker, Visibility vis) {
  ...
  return new EnumDecl(tracker, vis, identifier, std::move(ast_type_params),
                      std::move(option_decls));
}
...
const Item *Parser::parse_extern_block_or_fn_decl(Tracker tracker,
                                                  Visibility vis) {
  eat(Token::EXTERN);
  if (lookahead() == Token::FN)
    return parse_fn_decl(BodyMode::Mandatory, tracker, vis, /*extern*/ true,
                         /*abi*/ "");
  ...
  while (lookahead() == Token::FN)
    fn_decls.emplace_back(
        parse_fn_decl(BodyMode::None, tracker, vis, /*extern*/ true, abi));
  expect(Token::R_BRACE, "closing brace of external block");

  return new ExternBlock(tracker, vis, abi, std::move(fn_decls));
}

const FnDecl *Parser::parse_fn_decl(BodyMode mode, Tracker tracker,
                                    Visibility vis, bool is_extern,
                                    Symbol abi) {
  eat(Token::FN);
  auto export_name = lookahead() == Token::LIT_str ? lex().symbol() : Symbol();

  const Expr *filter =
      parse_filter("partial evaluation filter of function declaration");
  ...
  switch (mode) {
  case BodyMode::None:
    expect(Token::SEMICOLON, "function declaration");
    break;
  case BodyMode::Mandatory:
  ...
  return new FnDecl(tracker, vis, is_extern, abi, filter, export_name,
                    identifier, std::move(ast_type_params), std::move(params),
                    body);
}
...
const ImplItem *Parser::parse_impl(Tracker tracker, Visibility vis) {
  ...
  FnDecls methods;
  while (lookahead() == Token::FN)
    methods.emplace_back(parse_fn_decl(BodyMode::Mandatory, tracker, vis,
                                       /*exter*/ false, /*abi*/ ""));
  expect(Token::R_BRACE, "closing brace of impl");

  return new ImplItem(tracker, vis, std::move(ast_type_params), trait, ast_type,
                      std::move(methods));
}

const Item *Parser::parse_module_or_module_decl(Tracker tracker,
                                                Visibility vis) {
  eat(Token::MOD);
  ...
  if (accept(Token::L_BRACE)) {
    ...
    return new Module(tracker, vis, identifier, std::move(ast_type_params),
                      std::move(items));
  } else {
...
const StructDecl *Parser::parse_struct_decl(Tracker tracker, Visibility vis) {
  ...
  return new StructDecl(tracker, vis, identifier, std::move(ast_type_params),
...
const TraitDecl *Parser::parse_trait_decl(Tracker tracker, Visibility vis) {
  ...
  FnDecls methods;
  while (lookahead() == Token::FN)
    methods.emplace_back(parse_fn_decl(BodyMode::Optional, tracker, vis,
                                       /*exter*/ false, /*abi*/ ""));
  expect(Token::R_BRACE, "closing brace of trait declaration");

  return new TraitDecl(tracker, vis, identifier, std::move(ast_type_params),
                       std::move(super_traits), std::move(methods));
}
...
const Typedef *Parser::parse_typedef(Tracker tracker, Visibility vis) {
  ...
  return new Typedef(tracker, vis, identifier, std::move(ast_type_params),
                     ast_type);
}
...
const FnASTType *Parser::parse_fn_type() {
  ...
  return new FnASTType(tracker, std::move(ast_type_params),
                       std::move(ast_type_args));
}

const ASTType *Parser::parse_return_type(bool &is_continuation,
                                         bool mandatory) {
  auto tracker = track();
  ASTTypes ast_types;
...
const PtrASTType *Parser::parse_ptr_type() {
  ...
  if (accept(Token::ANDAND)) {
    ...
    return new PtrASTType(
        tracker, PtrASTType::Borrowed, 0,
        new PtrASTType(tracker, tag, addr_space, referenced_ast_type));
  }
...
const Expr *Parser::parse_expr(Prec prec) {
  auto tracker = track();
  auto lhs =
      lookahead().is_prefix() ? parse_prefix_expr() : parse_primary_expr();

  while (true) {
    /*
     * (lhs  op  LA) op ...  on break  (current prec > lhs prec of LA)  -->
     * reduce lhs  op (LA  op ...) otherwise -->  shift
     */
...
const Expr *Parser::parse_prefix_expr() {
  ...
  return new PrefixExpr(tracker, mut ? PrefixExpr::MUT : (PrefixExpr::Tag)tag,
                        rhs);
}
...
const MapExpr *Parser::parse_map_expr(Tracker tracker, const Expr *lhs) {
  eat(Token::L_PAREN);
  Exprs args;
  parse_comma_list("arguments of a map expression", Token::R_PAREN,
                   [&] { args.emplace_back(parse_expr()); });
  return new MapExpr(tracker, lhs, std::move(args));
}

const TypeAppExpr *Parser::parse_type_app_expr(Tracker tracker,
                                               const Expr *lhs) {
  eat(Token::L_BRACKET);
  ASTTypes ast_type_args;
  parse_comma_list("type arguments of a map expression", Token::R_BRACKET,
                   [&] { ast_type_args.emplace_back(parse_type()); });
...
const Expr *Parser::parse_postfix_expr(Tracker tracker, const Expr *lhs) {
  switch (lookahead()) {
  case Token::L_BRACKET:
    return parse_type_app_expr(tracker, lhs);
  case Token::L_PAREN:
    return parse_map_expr(tracker, lhs);
  case Token::DEC:
  case Token::INC: {
...
const Expr *Parser::parse_primary_expr() {
  ...
  case Token::L_PAREN: {
    ...
    if (accept(Token::COMMA)) {
      ...
      parse_comma_list("elements of a tuple expression", Token::R_PAREN,
                       [&] { args.emplace_back(parse_expr()); });
  ...
  case Token::L_BRACKET: {
    ...
    Exprs args;
    args.emplace_back(expr);
    parse_comma_list("elements of an array expression", Token::R_BRACKET,
                     [&] { args.emplace_back(parse_expr()); });
...
#include "impala/tokenlist.h"
  ...
  case Token::ID: {
    ...
    if (accept(Token::L_BRACKET)) { // struct or map expression
      parse_comma_list("type arguments", Token::R_BRACKET,
                       [&] { ast_type_args.emplace_back(parse_type()); });

      if (accept(Token::L_PAREN)) { // type app expression + map expression
        auto type_app_expr = new TypeAppExpr(tracker, new PathExpr(path),
                                             std::move(ast_type_args));
        Exprs args;
        parse_comma_list("arguments of a map expression", Token::R_PAREN,
                         [&] { args.emplace_back(parse_expr()); });
    ...
    if (lookahead(0) == Token::L_BRACE &&
      ...
      auto ast_type_app =
          new ASTTypeApp(tracker, path, std::move(ast_type_args));
...
const LiteralExpr *Parser::parse_literal_expr() {
  ...
  switch (lookahead()) {
  case Token::TRUE:
    return new LiteralExpr(lex().loc(), LiteralExpr::LIT_bool, 1);
  case Token::FALSE:
    return new LiteralExpr(lex().loc(), LiteralExpr::LIT_bool, 0);
...
char Parser::char_value(const char *&p) {
  ...
  if (*p++ == '\\') {
    ...
    // clang-format on
    default:
      // TODO make loc precise inside strings, reduce redundancy for single
      // chars
      impala::error(
          lookahead().loc(),
          "expected valid escape sequence, got '\\{}' while parsing {}",
          *(p - 1), lookahead());
    }
  } else
    value = *(p - 1);
...
const ForExpr *Parser::parse_for_expr() {
  ...
  auto break_decl =
      create_continuation_decl("break", /*set type during InferSema*/ false);
  return new ForExpr(tracker,
                     new FnExpr(tracker, filter, std::move(params), body), expr,
                     break_decl);
}
...
const ForExpr *Parser::parse_with_expr() {
  ...
  auto break_decl =
      create_continuation_decl("_", /*set type during InferSema*/ false);
  return new ForExpr(tracker,
                     new FnExpr(tracker, filter, std::move(params), body), expr,
                     break_decl);
}
