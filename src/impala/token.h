...
public:
  ...
  // add prefix and postfix tokens manually in order to avoid duplicates in the
  // enum
...
#include "impala/tokenlist.h"
  ...
  enum Op { None = 0, Prefix = 1, Infix = 2, Postfix = 4, Asgn_Op = 8 };
  ...
  static bool is_op(Tag tag) {
    return is_prefix(tag) || is_infix(tag) || is_postfix(tag);
  }
...
private:
  ...
  static Sym2Tag sym2lit_; ///< Table of \em all (including floating) suffixes
                           ///< for literals.
  static Sym2Tag
      sym2flit_; ///< Table of suffixes for \em floating point literals.
