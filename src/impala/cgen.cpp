...
private:
    ...
    template<typename F> void struct_from_type(const Type* type, const F& f) {
        if (type->isa<SimdType>())
            needs_vectors = true; // if the type mentions a vector, then we need to
        ...
        if (auto struct_type = type->isa<StructType>())
            ...
            for (size_t i = 0, e = fn_type->num_params(); i != e; ++i) struct_from_type(fn_type->param(i), f);
    }
    ...
    static bool ctype_from_impala(const Type* type, std::string& ctype_prefix, std::string& ctype_suffix) {
        if (auto prim_type = type->isa<PrimType>()) {
            switch (prim_type->primtype_tag()) {
                ...
                case PrimType_i32:
                    ...
                    return true;
                case PrimType_i64:
                    ctype_prefix = "long long";
                    ctype_suffix = "";
                    return true;
                case PrimType_u8:
                    ctype_prefix = "unsigned char";
                    ctype_suffix = "";
                    return true;
                case PrimType_u16:
                    ctype_prefix = "unsigned short";
                    ctype_suffix = "";
                    return true;
                case PrimType_u32:
                    ctype_prefix = "unsigned int";
                    ctype_suffix = "";
                    return true;
                case PrimType_u64:
                    ctype_prefix = "unsigned long long";
                    ctype_suffix = "";
                    return true;
                case PrimType_f16:
                    ctype_prefix = "half";
        ...
        if (auto simd_type = type->isa<SimdType>()) {
            ...
            switch (prim->primtype_tag()) {
                case PrimType_i32:
                    if (simd_type->dim() == 4)
                        ctype_prefix = "__m128i";
                    else if (simd_type->dim() == 8)
                        ctype_prefix = "__m256i";
                    else
                        return false;
                    break;
                case PrimType_f16: fe::unreachable();
                case PrimType_f32:
                    if (simd_type->dim() == 4)
                        ctype_prefix = "__m128";
                    else if (simd_type->dim() == 8)
                ...
                case PrimType_f64:
                    if (simd_type->dim() == 4)
                        ctype_prefix = "__m128d";
                    else if (simd_type->dim() == 8)
                        ctype_prefix = "__m256d";
                    else
                        return false;
                    break;
        ...
        // C void type is represented as an empty tuple (other tuples are not
        // supported for interface generation)
        ...
        if (auto ptr_type = type->isa<PtrType>()) {
            ...
            if (auto array_type = ptr_type->pointee()->isa<ArrayType>()) {
                if (!ctype_from_impala(array_type->elem_type(), ctype_prefix, ctype_suffix)) return false;
            } else {
                if (!ctype_from_impala(ptr_type->pointee(), ctype_prefix, ctype_suffix)) return false;
            }
        ...
        if (auto darray_type = type->isa<DefiniteArrayType>()) {
            if (!ctype_from_impala(darray_type->elem_type(), ctype_prefix, ctype_suffix)) return false;
            ctype_suffix = "[" + std::to_string(darray_type->dim()) + "]" + ctype_suffix;
            return true;
        }

        if (auto iarray_type = type->isa<IndefiniteArrayType>()) {
            if (!ctype_from_impala(iarray_type->elem_type(), ctype_prefix, ctype_suffix)) return false;
        ...
        for (const auto& field : cur_gen->field_decls()) {
            struct_from_type(field->type(), [&](const StructDecl* decl) {
                ...
                if (it != struct_decls.end()) {
                    if (it->second == NOT_GEN) compute_struct_order(struct_decls, order, decl);
                }
            });
        }
    ...
    void process_struct_decl(const StructDecl* struct_decl) {
        ...
        for (const auto& field : struct_decl->field_decls())
            struct_from_type(field->type(), [this](const StructDecl* decl) { export_structs.insert(decl); });
    }

    void process_fn_decl(const FnDecl* fn_decl, bool should_export = true) {
        // Read each argument in turn and record the structures that have to be
        // exported
        struct_from_type(fn_decl->fn_type(), [this](const StructDecl* decl) { export_structs.insert(decl); });
...
public:
    ...
    void process_module(const Module* mod) {
        for (const auto& item : mod->items()) {
            if (auto block = item->isa<ExternBlock>()) {
                if (block->abi().remove_quotation() != "C") continue;
                for (auto& decl : block->fn_decls()) process_fn_decl(decl.get(), false);
            }
    ...
    void add_dependencies() {
        ...
        do {
            ...
            for (auto st : exports) process_struct_decl(st);
        } while (exports.size() != export_structs.size());
    }

    bool generate_structs(std::ostream& o) {
        if (export_structs.empty()) return true;
        ...
        do {
            ...
            for (size_t i = prev_id, e = order.size(); i != e; i++) struct_decls.erase(order[i]);
    ...
    bool generate_functions(std::ostream& o) const {
        for (const auto& fn : export_fns) {
            ...
            // Generate all arguments except the last one which is the implicit
            // continuation
            for (size_t i = 0, e = fn_type->num_params() - 1; i != e; ++i) {
                ...
                if (i < fn_type->num_params() - 2) o << ", ";
            }

            // Generate void functions when the function takes no argument to be C89
            // compatible
            if (fn_type->num_params() == 1) o << "void";
...
bool generate_c_interface(const Module* mod, const CGenOptions& opts, std::ostream& o) {
    if (opts.fns_only && opts.structs_only) return false;
    ...
    if (cgen.needs_vectors) o << "#include <immintrin.h>\n" << std::endl;

    // Export structures
    if (!opts.fns_only && !cgen.generate_structs(o)) return false;

    // Export functions
    if (!opts.structs_only && !cgen.generate_functions(o)) return false;
