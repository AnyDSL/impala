...
private:
  ...
  static bool ctype_from_impala(const Type *type, std::string &ctype_prefix,
                                std::string &ctype_suffix) {
    ...
    if (auto ptr_type = type->isa<PtrType>()) {
      ...
      if (auto array_type = ptr_type->pointee()->isa<ArrayType>()) {
        if (!ctype_from_impala(array_type->elem_type(), ctype_prefix,
                               ctype_suffix))
          return false;
      } else {
    ...
    if (auto darray_type = type->isa<DefiniteArrayType>()) {
      if (!ctype_from_impala(darray_type->elem_type(), ctype_prefix,
                             ctype_suffix))
        return false;
      ctype_suffix =
          "[" + std::to_string(darray_type->dim()) + "]" + ctype_suffix;
      return true;
    }

    if (auto iarray_type = type->isa<IndefiniteArrayType>()) {
      if (!ctype_from_impala(iarray_type->elem_type(), ctype_prefix,
                             ctype_suffix))
        return false;
  ...
  void compute_struct_order(
      thorin::GIDMap<const StructDecl *, GenState> &struct_decls,
      std::vector<const StructDecl *> &order, const StructDecl *cur_gen) {
...
bool generate_c_interface(const Module *mod, const CGenOptions &opts,
                          std::ostream &o) {
  ...
  o << "/* " << opts.file_name
    << " : Impala interface file generated by impala */\n"
    << "#ifndef " << opts.guard << "\n"
    << "#define " << opts.guard << "\n\n"
    << "#ifdef __cplusplus\n"
    << "extern \"C\" {\n"
