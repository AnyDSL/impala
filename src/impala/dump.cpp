...
std::ostream& FnTypeNode::print(Printer& p) const {
    ...
    p.dump_list([&] (Type type) { p.stream() << type; }, args().skip_back(1), "(", ")");
    p.stream() << " -> ";
...
std::ostream& TraitAppNode::print(Printer& p) const {
    ...
    p.stream() << trait();
    if (num_args() > 1)
        return p.dump_list([&] (Type type) { type->print(p); }, args().skip_front(1), "[", "]");
    return p.stream();
}
...
std::ostream& FnASTType::print(Printer& p) const {
    ...
    p.stream() << "fn";
    print_type_params(p);
    p.dump_list([&] (const ASTType* arg) { arg->print(p); }, ret != nullptr ? args().skip_back(1) : args(), "(", ")");
    if (ret != nullptr) {
        p.stream() << " -> ";
        if (ret->num_args() == 1) {
...
std::ostream& Fn::print_params(Printer& p, bool returning) const {
    return p.dump_list([&] (const Param* param) {
        ...
        returning ? params().skip_back(1) : params());
}
...
std::ostream& ForExpr::print(Printer& p) const {
    p.stream() << "for ";
    p.dump_list([&](const Param* param) { param->print(p); }, fn_expr()->params().skip_back(1)) << " in ";
    p.print(expr()) << ' ';
