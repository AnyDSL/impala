...
class Visibility {
public:
  enum { None, Pub = Token::PUB, Priv = Token::PRIV };

  Visibility(int visibility) : visibility_(visibility) {}
...
/// Mixin for all entities which have a list of @p TypeParam%s: [T1, T2 : A +
/// B[...], ...].
...
public:
  ...
  const ASTTypeParam *ast_type_param(size_t i) const {
    return ast_type_params_[i].get();
  }
...
template <class... Args>
void warning(const ASTNode *n, const char *fmt, Args... args) {
  impala::warning(n->loc(), fmt, args...);
}
template <class... Args>
void error(const ASTNode *n, const char *fmt, Args... args) {
  impala::error(n->loc(), fmt, args...);
}

class Identifier : public ASTNode {
public:
  Identifier(Loc loc, Symbol symbol) : ASTNode(loc), symbol_(symbol) {}
  Identifier(Token tok) : ASTNode(tok.loc()), symbol_(tok.symbol()) {}
...
public:
  ...
  public:
    Elem(const Identifier *id) : Typeable(id->loc()), identifier_(id) {}
  ...
  Path(Loc loc, bool global, Elems &&elems)
      : Typeable(loc), global_(global), elems_(std::move(elems)) {}
  Path(const Identifier *id) : Path(id->loc(), false, Elems()) {
...
public:
  ErrorASTType(Loc loc) : ASTType(loc) {}
...
#include "impala/tokenlist.h"
  ...
  PrimASTType(Loc loc, Tag tag) : ASTType(loc), tag_(tag) {}
...
public:
  ...
  PtrASTType(Loc loc, Tag tag, int addr_space,
             const ASTType *referenced_ast_type)
      : ASTType(loc), tag_(tag), addr_space_(addr_space),
        referenced_ast_type_(referenced_ast_type) {}

  Tag tag() const { return tag_; }
  std::string prefix() const;
  const ASTType *referenced_ast_type() const {
    return referenced_ast_type_.get();
  }
...
public:
  ArrayASTType(Loc loc, const ASTType *elem_ast_type)
      : ASTType(loc), elem_ast_type_(elem_ast_type) {}
...
public:
  DefiniteArrayASTType(Loc loc, const ASTType *elem_ast_type, uint64_t dim)
      : ArrayASTType(loc, elem_ast_type), dim_(dim) {}
...
public:
  CompoundASTType(Loc loc, ASTTypes &&ast_type_args)
      : ASTType(loc), ast_type_args_(std::move(ast_type_args)) {}

  size_t num_ast_type_args() const { return ast_type_args_.size(); }
  ASTTypeArgs ast_type_args() const { return ast_type_args_; }
  const ASTType *ast_type_arg(size_t i) const {
    return ast_type_args_[i].get();
  }
...
public:
  ASTTypeApp(Loc loc, const Path *path, ASTTypes &&ast_type_args)
      : CompoundASTType(loc, std::move(ast_type_args)), path_(path) {}

  ASTTypeApp(Loc loc, const Path *path) : ASTTypeApp(loc, path, ASTTypes()) {}

  const Path *path() const { return path_.get(); }
  const Identifier *identifier() const {
    return path()->elems().back()->identifier();
  }
  Symbol symbol() const { return identifier()->symbol(); }
...
public:
  Typeof(Loc loc, const Expr *expr) : ASTType(loc), expr_(dock(expr_, expr)) {}
...
public:
  enum Tag {
    ...
    TypeDecl,     ///< Represents a type definition like @p StructDecl or @p
                  ///< Typedef.
    ValueDecl,    ///< Represents something which is a value at run time.
  };

  /// General constructor.
  Decl(Tag tag, Loc loc, bool mut, const Identifier *id,
       const ASTType *ast_type)
      : Typeable(loc), tag_(tag), identifier_(id), ast_type_(ast_type),
        mut_(mut), written_(false) {}
  /// @p NoDecl.
  Decl(Loc loc) : Decl(NoDecl, loc, false, nullptr, nullptr) {}
  ...
  Symbol symbol() const {
    assert(!is_no_decl());
    return identifier_->symbol();
  }
  bool is_anonymous() const {
    assert(!is_no_decl());
    return symbol() == Symbol() || symbol().c_str()[0] == '<';
  }
  size_t depth() const {
    assert(!is_no_decl());
    return depth_;
  }
  const Decl *shadows() const {
    assert(!is_no_decl());
    return shadows_;
  }
  ...
  const ASTType *ast_type() const {
    assert(is_value_decl());
    return ast_type_.get();
  } ///< Original @p ASTType.
  bool is_mut() const {
    assert(is_value_decl());
    return mut_;
  ...
  const mim::Def *def() const { return def_; }
...
protected:
  mutable const mim::Def *def_ = nullptr;
...
public:
  ...
  void emit(CodeGen &, const mim::Def *) const;
  void bind(NameSema &) const;
...
public:
  ASTTypeParam(Loc loc, const Identifier *id, ASTTypes &&bounds)
      : Decl(TypeDecl, loc, id), bounds_(std::move(bounds)) {}
...
public:
  Fn(const Expr *filter, ASTTypeParams &&ast_type_params, Params &&params,
     const Expr *body)
      : ASTTypeParamList(std::move(ast_type_params)),
        filter_(dock(filter_, filter)), params_(std::move(params)),
  ...
  mim::Lam *lam() const { return lam_; }
  const mim::Def *ret_param() const { return ret_param_; }
  const mim::Def *frame() const { return frame_; }
  Stream &stream_params(Stream &p, bool returning) const;
  void fn_bind(NameSema &) const;
  const Type *check_body(TypeSema &) const;
  mim::Lam *fn_emit_head(CodeGen &, Loc) const;
...
protected:
  ...
  Params params_;
  mutable mim::Lam *lam_ = nullptr;
  mutable const mim::Def *ret_param_ = nullptr;
  mutable const mim::Def *frame_ = nullptr;
...
public:
  /// @p NoDecl.
  Item(Loc loc, Visibility vis) : Decl(loc), visibility_(vis) {}

  /// @p TypeableDecl, @p TypeDecl or @p ValueDecl.
  Item(Tag tag, Loc loc, Visibility vis, const Identifier *id)
      : Decl(tag, loc, id), visibility_(vis) {}

  /// @p ValueDecl.
  Item(Loc loc, Visibility vis, bool mut, const Identifier *id,
       const ASTType *ast_type)
      : Decl(ValueDecl, loc, mut, id, ast_type), visibility_(vis) {}
...
public:
  TypeDeclItem(Loc loc, Visibility vis, const Identifier *id,
...
public:
  ValueItem(Loc loc, Visibility vis, bool mut, const Identifier *id,
            const ASTType *ast_type)
...
public:
  Module(Loc loc, Visibility vis, const Identifier *id,
  ...
  Module(const std::filesystem::path *first_file_name, Items &&items = Items())
      : Module(items.empty()
                   ? Loc(first_file_name, 1, 1)
                   : Loc(items.front()->loc().path, items.front()->loc().begin,
                         items.back()->loc().finis),
...
public:
  ModuleDecl(Loc loc, Visibility vis, const Identifier *id,
             ASTTypeParams &&ast_type_params)
      : TypeDeclItem(loc, vis, id, std::move(ast_type_params)) {}
...
public:
  ExternBlock(Loc loc, Visibility vis, Symbol abi, FnDecls &&fn_decls)
      : Item(loc, vis), abi_(abi), fn_decls_(std::move(fn_decls)) {}
...
public:
  FieldDecl(Loc loc, size_t index, Visibility vis, const Identifier *id,
            const ASTType *ast_type)
      : Decl(TypeableDecl, loc, id), index_(index), visibility_(vis),
...
public:
  ...
  std::optional<const FieldDecl *> field_decl(Symbol symbol) const {
    if (auto i = field_table_.find(symbol); i != field_table_.end())
      return i->second;
    return {};
  }
  std::optional<const FieldDecl *> field_decl(const Identifier *ident) const {
    return field_decl(ident->symbol());
  }
...
public:
  OptionDecl(Loc loc, size_t index, const Identifier *id, ASTTypes args)
      : Decl(ValueDecl, loc, id), index_(index), args_(std::move(args)) {}
  ...
  const mim::Def *variant_type(CodeGen &) const;
...
public:
  ...
  const OptionDecl *option_decl(size_t i) const {
    return option_decls_[i].get();
  }
  std::optional<const OptionDecl *> option_decl(Symbol symbol) const {
    if (auto i = option_table_.find(symbol); i != option_table_.end())
      return i->second;
    return {};
  }
...
public:
  FnDecl(Loc loc, Visibility vis, bool is_extern, Symbol abi,
         const Expr *filter, Symbol export_name, const Identifier *id,
         ASTTypeParams &&ast_type_params, Params &&params, const Expr *body)
      : ValueItem(loc, vis, /*mut*/ false, id, /*ast_type*/ nullptr),
        Fn(filter, std::move(ast_type_params), std::move(params), body),
        abi_(abi), export_name_(export_name), is_extern_(is_extern) {}
  ...
  Symbol fn_symbol() const override {
    return export_name_ != "" ? export_name_ : identifier()->symbol();
  }
...
public:
  TraitDecl(Loc loc, Visibility vis, const Identifier *id,
            ASTTypeParams &&ast_type_params, ASTTypeApps &&super_traits,
            FnDecls &&methods)
      : Item(TypeDecl, loc, vis, id),
        ASTTypeParamList(std::move(ast_type_params)),
        super_traits_(std::move(super_traits)), methods_(std::move(methods)) {}
...
public:
  ImplItem(Loc loc, Visibility vis, ASTTypeParams &&ast_type_params,
      ...
      : Item(loc, vis), ASTTypeParamList(std::move(ast_type_params)),
        trait_(std::move(trait)), ast_type_(std::move(ast_type)),
...
public:
  ...
  virtual void bind(NameSema &) const = 0;
  virtual const mim::Def *lemit(CodeGen &) const;
  virtual const mim::Def *remit(CodeGen &) const;
  virtual void emit_branch(CodeGen &, mim::Lam *, mim::Lam *) const;
...
protected:
   ...
   * This means that the address is @em not supposed to be changed in the
   * future. For this reason, @p Exprs is a <tt>
   * std::deque<std::unique_ptr<const Expr>> </tt> and @em not a @c std::vector.
   */
 ...
 * while @p expr gets released from its @p Expr::back_ref_ and the newly created
 * node is @p expr's new owner. This means that @p expr also occurs within @p
 * args.
 */
template <class T, class... Args>
...
class Args {
public:
  Args(Exprs &&args) : args_(std::move(args)) {
    for (auto &arg : args_)
      arg->back_ref_ = &arg;
  }

  const Exprs &args() const { return args_; }
  const Expr *arg(size_t i) const {
    assert(i < args_.size());
    return args_[i].get();
  }
  size_t num_args() const { return args_.size(); }
...
private:
  const Type *infer(InferSema &) const override;
  void check(TypeSema &) const override;
  const mim::Def *remit(CodeGen &) const override;
};
...
#include "impala/tokenlist.h"
  ...
  LiteralExpr(Loc loc, Tag tag, uint64_t val)
      : Expr(loc), tag_(tag), val_(val) {}

  Tag tag() const { return tag_; }
  template <class T = uint64_t> T get() const { return mim::bitcast<T>(val_); }
...
public:
  CharExpr(Loc loc, Symbol symbol, char value)
      : Expr(loc), symbol_(symbol), value_(value) {}
...
public:
  StrExpr(Loc loc, Symbols &&symbols, std::vector<char> &&values)
      : Expr(loc), symbols_(std::move(symbols)), values_(std::move(values)) {}
...
public:
  FnExpr(Loc loc, const Expr *filter, Params &&params, const Expr *body)
      : Expr(loc), Fn(filter, ASTTypeParams(), std::move(params), body) {}
...
public:
  PathExpr(const Path *path) : Expr(path->loc()), path_(path) {}
  PathExpr(const Identifier *identifier) : PathExpr(new Path(identifier)) {}

  const Path *path() const { return path_.get(); }
  const Decl *value_decl() const {
    return path_->decl() && path_->decl()->is_value_decl() ? path_->decl()
                                                           : nullptr;
  }
...
private:
  const Type *infer(InferSema &) const override;
  void check(TypeSema &) const override;
  const mim::Def *lemit(CodeGen &) const override;
...
#include "impala/tokenlist.h"
  ...
  PrefixExpr(Loc loc, Tag tag, const Expr *rhs)
      : Expr(loc), tag_(tag), rhs_(dock(rhs_, rhs)) {}

  static const PrefixExpr *create(const Expr *rhs, const Tag tag) {
    return interlope<PrefixExpr>(rhs, rhs->loc(), tag, rhs);
  }
  static const PrefixExpr *create_deref(const Expr *rhs) {
    return create(rhs, MUL);
  }
...
#include "impala/tokenlist.h"
  ...
  InfixExpr(Loc loc, const Expr *lhs, Tag tag, const Expr *rhs)
      : Expr(loc), tag_(tag), lhs_(dock(lhs_, lhs)), rhs_(dock(rhs_, rhs)) {}
  ...
  void bind(NameSema &) const override;
  const mim::Def *remit(CodeGen &) const override;
  void emit_branch(CodeGen &, mim::Lam *, mim::Lam *) const override;
  Stream &stream(Stream &) const override;
...
public:
  enum Tag { INC = Token::INC, DEC = Token::DEC };

  PostfixExpr(Loc loc, const Expr *lhs, Tag tag)
      : Expr(loc), tag_(tag), lhs_(dock(lhs_, lhs)) {}
...
public:
  FieldExpr(Loc loc, const Expr *lhs, const Identifier *id)
      : Expr(loc), lhs_(dock(lhs_, lhs)), identifier_(id) {}
...
public:
  CastExpr(Loc loc, const Expr *src) : Expr(loc), src_(dock(src_, src)) {}
...
public:
  ExplicitCastExpr(Loc loc, const Expr *src, const ASTType *ast_type)
      : CastExpr(loc, src), ast_type_(ast_type) {}
...
public:
  RValueExpr(const Expr *src) : CastExpr(src->loc(), src) {}
...
public:
  DefiniteArrayExpr(Loc loc, Exprs &&args) : Expr(loc), Args(std::move(args)) {}
...
public:
  RepeatedDefiniteArrayExpr(Loc loc, const Expr *value, uint64_t count)
      : Expr(loc), value_(dock(value_, value)), count_(count) {}
...
public:
  IndefiniteArrayExpr(Loc loc, const Expr *dim, const ASTType *elem_ast_type)
      : Expr(loc), dim_(dock(dim_, dim)), elem_ast_type_(elem_ast_type) {}
...
public:
  TupleExpr(Loc loc, Exprs &&args) : Expr(loc), Args(std::move(args)) {}
...
public:
  ...
  public:
    Elem(Loc loc, const Identifier *id, const Expr *expr)
        : ASTNode(loc), identifier_(id), expr_(dock(expr_, expr)) {}
  ...
  StructExpr(Loc loc, const ASTTypeApp *ast_type_app, Elems &&elems)
      : Expr(loc), ast_type_app_(ast_type_app), elems_(std::move(elems)) {}
...
public:
  MapExpr(Loc loc, const Expr *lhs, Exprs &&args)
      : Expr(loc), Args(std::move(args)), lhs_(dock(lhs_, lhs)) {}
...
public:
  BlockExpr(Loc loc, Stmts &&stmts, const Expr *expr)
      : Expr(loc), stmts_(std::move(stmts)), expr_(dock(expr_, expr)) {}
  /// An empty BlockExpr with no @p stmts and an @p EmptyExpr as @p expr.
  BlockExpr(Loc loc) : BlockExpr(loc, Stmts(), new EmptyExpr(loc)) {}
...
protected:
  ...
  std::unique_ptr<const Expr> expr_;
  mutable LocalDecls
      locals_; ///< All @p LocalDecl%s in this @p BlockExpr from top to bottom.
};

class IfExpr : public Expr {
public:
  IfExpr(Loc loc, const Expr *cond, const Expr *then_expr,
         const Expr *else_expr)
      : Expr(loc), cond_(dock(cond_, cond)),
...
public:
  ...
  public:
    Arm(Loc loc, const Ptrn *ptrn, const Expr *expr)
        : ASTNode(loc), ptrn_(ptrn), expr_(dock(expr_, expr)) {}
  ...
  MatchExpr(Loc loc, const Expr *expr, Arms &&arms)
      : Expr(loc), expr_(dock(expr_, expr)), arms_(std::move(arms)) {}
...
public:
  WhileExpr(Loc loc, const LocalDecl *continue_decl, const Expr *cond,
      ...
      : Expr(loc), continue_decl_(continue_decl), cond_(dock(cond_, cond)),
        body_(dock(body_, body)), break_decl_(break_decl) {}
...
public:
  ForExpr(Loc loc, const Expr *fn_expr, const Expr *expr,
          const LocalDecl *break_decl)
      : Expr(loc), fn_expr_(dock(fn_expr_, fn_expr)), expr_(dock(expr_, expr)),
...
public:
  RevDiffExpr(Loc loc, const Expr *expr)
      : Expr(loc), expr_(dock(expr_, expr)) {}
...
public:
  Ptrn(Loc loc) : Typeable(loc) {}

  virtual void bind(NameSema &) const = 0;
  virtual void emit(CodeGen &, const mim::Def *) const = 0;
  virtual const mim::Def *emit(CodeGen &) const { return nullptr; }
  virtual const mim::Def *emit_cond(CodeGen &, const mim::Def *) const = 0;
...
public:
  TuplePtrn(Loc loc, Ptrns &&elems) : Ptrn(loc), elems_(std::move(elems)) {}
  ...
  void bind(NameSema &) const override;
  void emit(CodeGen &, const mim::Def *) const override;
  const mim::Def *emit_cond(CodeGen &, const mim::Def *) const override;
...
public:
  IdPtrn(const LocalDecl *local) : Ptrn(local->loc()), local_(local) {}
...
public:
  EnumPtrn(Loc loc, const Path *path, Ptrns &&args)
      : Ptrn(loc), path_(path), args_(std::move(args)) {}
...
public:
  LiteralPtrn(const LiteralExpr *literal, bool minus)
      : Ptrn(literal->loc()), literal_(dock(literal_, literal)), minus_(minus) {
  }

  const LiteralExpr *literal() const {
    return literal_.get()->as<LiteralExpr>();
  }
  bool has_minus() const { return minus_; }

  void bind(NameSema &) const override;
  void emit(CodeGen &, const mim::Def *) const override;
  const mim::Def *emit(CodeGen &) const override;
  const mim::Def *emit_cond(CodeGen &, const mim::Def *) const override;
  bool is_refutable() const override;
  Stream &stream(Stream &) const override;
...
public:
  CharPtrn(const CharExpr *chr) : Ptrn(chr->loc()), chr_(dock(chr_, chr)) {}
...
public:
  ExprStmt(Loc loc, const Expr *expr) : Stmt(loc), expr_(dock(expr_, expr)) {}
...
public:
  ItemStmt(Loc loc, const Item *item) : Stmt(loc), item_(item) {}
...
public:
  LetStmt(Loc loc, const Ptrn *ptrn, const Expr *init)
      : Stmt(loc), ptrn_(ptrn), init_(dock(init_, init)) {}
...
public:
  ...
  public:
    Elem(Loc loc, std::string &&constraint, const Expr *expr)
        : ASTNode(loc), constraint_(std::move(constraint)),
          expr_(dock(expr_, expr)) {}
  ...
  AsmStmt(Loc loc, std::string &&asm_template, Elems &&outputs, Elems &&inputs,
      ...
      : Stmt(loc), asm_template_(std::move(asm_template)),
        outputs_(std::move(outputs)), inputs_(std::move(inputs)),
        clobbers_(std::move(clobbers)), options_(std::move(options)) {}
