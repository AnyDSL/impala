...
template<class... Args> void warning(const ASTNode* n, const char* fmt, Args... args) {
    impala::warning(n->loc(), fmt, args...);
}
template<class... Args> void error(const ASTNode* n, const char* fmt, Args... args) {
...
public:
    ...
    Module(const std::filesystem::path* first_file_name, Items&& items = Items())
        : Module(items.empty() ? Loc(first_file_name, 1, 1)
                               : Loc(items.front()->loc().path, items.front()->loc().begin, items.back()->loc().finis),
                 Visibility::Pub,
                 nullptr,
                 ASTTypeParams(),
                 std::move(items)) {}
...
public:
    Typedef(Loc loc, Visibility vis, const Identifier* id, ASTTypeParams&& ast_type_params, const ASTType* ast_type)
        : TypeDeclItem(loc, vis, id, std::move(ast_type_params))
        , ast_type_(ast_type) {}
...
public:
    StructDecl(Loc loc, Visibility vis, const Identifier* id, ASTTypeParams&& ast_type_params, FieldDecls&& field_decls)
        : TypeDeclItem(loc, vis, id, std::move(ast_type_params))
...
public:
    EnumDecl(Loc loc, Visibility vis, const Identifier* id, ASTTypeParams&& ast_type_params, OptionDecls&& option_decls)
        : TypeDeclItem(loc, vis, id, std::move(ast_type_params))
        , option_decls_(std::move(option_decls)) {
        for (auto& option : option_decls_) simple_ &= option->num_args() == 0;
    }
...
public:
    StaticItem(Loc loc, Visibility vis, bool mut, const Identifier* id, const ASTType* ast_type, const Expr* init)
...
public:
    ...
    const FnType* fn_type() const override {
        auto t = type();
        while (auto lambda = t->isa<Lambda>()) t = lambda->body();
        return t->as<FnType>();
    }
...
protected:
    ...
    template<class T, class... Args> friend const T* interlope(const Expr* expr, Args&&... args);
 ...
 * args.
 */
template<class T, class... Args> const T* interlope(const Expr* expr, Args&&... args) {
...
public:
    Args(Exprs&& args)
        : args_(std::move(args)) {
        for (auto& arg : args_) arg->back_ref_ = &arg;
    }
...
public:
    ...
    const Path* path() const { return path_.get(); }
    const Decl* value_decl() const { return path_->decl() && path_->decl()->is_value_decl() ? path_->decl() : nullptr; }
...
public:
    ...
    static const RValueExpr* create(const Expr* src) { return interlope<RValueExpr>(src, src); }
...
public:
    WhileExpr(Loc loc, const LocalDecl* continue_decl, const Expr* cond, const Expr* body, const LocalDecl* break_decl)
        : Expr(loc)
...
public:
    ...
    AsmStmt(Loc loc, std::string&& asm_template, Elems&& outputs, Elems&& inputs, Strings&& clobbers, Strings&& options)
        : Stmt(loc)
    ...
    Array<std::string> output_constraints() const {
        ...
        for (size_t i = 0, e = result.size(); i != e; ++i) result[i] = output(i)->constraint();
        return result;
    }

    Array<std::string> input_constraints() const {
        Array<std::string> result(num_inputs());
        for (size_t i = 0, e = result.size(); i != e; ++i) result[i] = input(i)->constraint();
        return result;
    }
