...
ASTNode::ASTNode(Loc loc) : gid_(gid_counter_++), loc_(loc) {}
...
bool MapExpr::has_side_effect() const {
  return bool(lhs()->type()->isa<FnType>());
}
bool BlockExpr::has_side_effect() const {
  return !stmts().empty() || expr()->has_side_effect();
}

bool IfExpr::has_side_effect() const {
  return cond()->has_side_effect() || then_expr()->has_side_effect() ||
         else_expr()->has_side_effect();
}

bool MatchExpr::has_side_effect() const {
  return expr()->has_side_effect() ||
         std::any_of(arms().begin(), arms().end(),
                     [](const std::unique_ptr<const Arm> &arm) {
                       return arm->expr()->has_side_effect();
                     });
}
