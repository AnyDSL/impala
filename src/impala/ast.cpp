...
const FnASTType* FnASTType::ret_fn_ast_type() const {
    if (num_ast_type_args() != 0) {
        if (auto fn_type = ast_type_args().back()->isa<FnASTType>()) return fn_type;
    }
    return nullptr;
}
...
bool IfExpr::has_else() const {
    if (auto block = else_expr_->isa<BlockExpr>()) return !block->empty();
    return true;
}
...
void PathExpr::write() const {
    if (value_decl()) value_decl()->write();
}

void MapExpr::write() const {
    auto type = unpack_ref_type(lhs()->type());
    if (type->isa<ArrayType>() || type->isa<TupleType>() || type->isa<PtrType>()) lhs()->write();
}

void PrefixExpr::write() const {
    if (tag() == MUT) rhs()->write();
}
...
bool RValueExpr::has_side_effect() const { return src()->has_side_effect(); }

bool PrefixExpr::has_side_effect() const { return tag() == INC || tag() == DEC || tag() == TILDE || tag() == HLT; }

bool InfixExpr::has_side_effect() const { return Token::is_assign((TokenTag)tag()); }
...
void PathExpr::take_address() const {
    if (value_decl()) {
        if (auto local = value_decl()->isa<LocalDecl>()) local->take_address();
    }
}
...
const PrefixExpr* PrefixExpr::create_addrof(const Expr* rhs) {
    if (auto rvalue = rhs->isa<RValueExpr>()) return replace_rvalue_by_addrof(rvalue);
