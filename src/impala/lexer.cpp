...
Token Lexer::lex() {
    while (true) {
        ...
        // end of file
        if (accept(std::istream::traits_type::eof())) return {loc(), Token::Eof};
...
#define IMPALA_WITHIN_COMMENT(delim)                    \
    while (true) {                                      \
        ...
        if (accept('/')) {
            if (accept('=')) return {loc(), Token::DIV_ASGN};
...
#define IMPALA_LEX_AND_OR(op, tok_bit, tok_logic, tok_asgn) \
    if (accept(op)) {                                       \
        if (accept('=')) return {loc(), Token::tok_asgn};   \
        if (accept(op)) return {loc(), Token::tok_logic};   \
        ...
        if (accept(':')) {
            if (accept(':')) return {loc(), Token::DOUBLE_COLON};
            return {loc(), Token::COLON};
        }

        if (accept('@')) {
            if (accept('@')) return {loc(), Token::RUNRUN};
            if (accept('?')) return {loc(), Token::RUNKNOWN};
        ...
        if (lex_identifier(str)) return {loc(), str};
