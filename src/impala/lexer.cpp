...
Lexer::Lexer(std::istream &stream, const std::filesystem::path *filename)
...
Token Lexer::lex() {
  while (true) {
    ...
    if (accept('+')) {
      if (accept('+'))
        return {loc(), Token::INC};
      if (accept('='))
        return {loc(), Token::ADD_ASGN};
    ...
    if (accept('-')) {
      if (accept('-'))
        return {loc(), Token::DEC};
      if (accept('='))
        return {loc(), Token::SUB_ASGN};
      if (accept('>'))
        return {loc(), Token::ARROW};
...
#define IMPALA_LEX_OP(op, tok1, tok2)                                          \
  if (accept(op)) {                                                            \
    ...
    // <, <=, <<, <<=, >, >=, >>, >>=
#define IMPALA_LEX_REL_SHIFT(op, tok_rel, tok_rel_eq, tok_shift,               \
                             tok_shift_asgn)                                   \
  if (accept(op)) {                                                            \
    if (accept('='))                                                           \
      return {loc(), Token::tok_rel_eq};                                       \
    if (accept(op)) {                                                          \
      if (accept('='))                                                         \
        return {loc(), Token::tok_shift_asgn};                                 \
...
#define IMPALA_LEX_AND_OR(op, tok_bit, tok_logic, tok_asgn)                    \
  if (accept(op)) {                                                            \
    ...
    if (accept('('))
      return {loc(), Token::L_PAREN};
    if (accept(')'))
      return {loc(), Token::R_PAREN};
    if (accept(','))
      return {loc(), Token::COMMA};
    if (accept(';'))
      return {loc(), Token::SEMICOLON};
    if (accept('$'))
      return {loc(), Token::HLT};
    if (accept('['))
      return {loc(), Token::L_BRACKET};
    if (accept(']'))
      return {loc(), Token::R_BRACKET};
    if (accept('{'))
      return {loc(), Token::L_BRACE};
    if (accept('}'))
      return {loc(), Token::R_BRACE};
    if (accept('~'))
      return {loc(), Token::TILDE};
    if (accept('?'))
      return {loc(), Token::KNOWN};

    // '.', floats
    if (accept('.')) {
      str += '.';
      if (accept(str, dec))
        goto l_fractional_dot_rest;
      if (accept('.'))
        return {loc(), Token::DOTDOT};
