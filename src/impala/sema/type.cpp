...
bool is_void(const Type* type) {
    if (auto t = type->isa<TupleType>()) return t->empty();
    return false;
}

const Type* FnType::param(size_t i) const { return op(0)->isa<TupleType>() ? op(0)->op(i) : op(i); }

size_t FnType::num_params() const { return op(0)->isa<TupleType>() ? op(0)->num_ops() : 1; }

const Type* FnType::last_param() const { return op(0)->isa<TupleType>() ? op(0)->ops().back() : op(0); }

bool FnType::is_returning() const {
    if (auto tuple = op(0)->isa<TupleType>()) {
        if (tuple->num_ops() == 0) return false;
        if (auto fn_type = tuple->ops().back()->isa<FnType>()) return !fn_type->is_returning();
        return false;
    } else {
...
const Type* FnType::return_type() const {
    ...
    if (last_param->isa<TupleType>()) {
        if (last_param->empty()) return table().type_noret();
        last_param = last_param->ops().back();
    }
    if (auto fn = last_param->isa<FnType>()) return fn->op(0);
...
bool is_subtype(const Type* dst, const Type* src) {
    ...
    if (dst->tag() == src->tag() && dst->num_ops() == src->num_ops()) {
        ...
        if (auto dst_fn = dst->isa<FnType>()) {
            ...
            for (size_t i = 0, e = dst->num_ops(); result && i != e; ++i) result &= is_subtype(dst->op(i), src->op(i));
        }
...
bool is_strict_subtype(const Type* dst, const Type* src) { return dst != src && is_subtype(dst, src); }
...
bool RefTypeBase::equal(const Type* other) const {
    return Type::equal(other) && this->is_mut() == other->as<RefTypeBase>()->is_mut()
...
#define IMPALA_TYPE(itype, atype) \
    case PrimType_##itype: return os << #itype;
...
const Type* DefiniteArrayType ::vrebuild(TypeTable& to, Types ops) const {
    return to.definite_array_type(ops[0], dim());
}
...
const Type* BorrowedPtrType ::vrebuild(TypeTable& to, Types ops) const {
    return to.borrowed_ptr_type(ops[0], is_mut(), addr_space());
}
...
const Type* StructType::vreduce(int depth, const Type* type, Type2Type& map) const {
    ...
    for (size_t i = 0, e = num_ops(); i != e; ++i) struct_type->set(i, op(i)->reduce(depth, type, map));
    return struct_type;
}
...
const Type* EnumType::vreduce(int depth, const Type* type, Type2Type& map) const {
    ...
    map[this]      = enum_type;
    for (size_t i = 0, e = num_ops(); i != e; ++i) enum_type->set(i, op(i)->reduce(depth, type, map));
    return enum_type;
}
...
const Type* TypeTable::app(const Type* callee, const Type* op) {
    ...
    if (auto cache = app->cache_) return cache;
...
const PrimType* TypeTable::prim_type(const PrimTypeTag tag) {
    switch (tag) {
#define IMPALA_TYPE(itype, atype) \
    case PrimType_##itype: return itype##_;
#include "impala/tokenlist.h"
        default: fe::unreachable();
    }
}
...
const InferError* TypeTable::infer_error(const Type* dst, const Type* src) {
    if (auto di = dst->isa<InferError>()) {
        if (di->src() == src) return di;
    }

    if (auto si = src->isa<InferError>()) {
        if (si->dst() == dst) return si;
    }
...
const Type* FnType::rev_diffed_type() const {
    ...
    if (auto t = params_without_return_continuation()->isa<TupleType>()) {
        ...
        for (size_t i = 0, e = t->num_ops(); i < e; ++i) params[i] = t->op(i);
...
const Type* PrimType::tangent_vector() const { return is_float(this) ? this : nullptr; }
...
const Type* IndefiniteArrayType::tangent_vector() const {
    ...
    return elem_tangent_vector != nullptr ? table().indefinite_array_type(elem_tangent_vector) : nullptr;
}
