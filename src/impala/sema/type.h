...
class TypeTable;
using Type = mim::TypeBase<TypeTable>;

class StructDecl;
class EnumDecl;
template <class T> using Span = mim::Span<T>;
template <class T> using Array = mim::Vector<T>;

template <class T> using TypeMap = mim::GIDMap<const Type *, T>;
using TypeSet = mim::GIDSet<const Type *>;
...
#define IMPALA_TYPE(itype, atype)                                              \
  inline bool is_##itype(const Type *t) { return is(t, PrimType_##itype); }
#include "impala/tokenlist.h"
inline bool is_float(const Type *t) {
  return is_f16(t) || is_f32(t) || is_f64(t);
}
inline bool is_int(const Type *t) {
  return is_i8(t) || is_i16(t) || is_i32(t) || is_i64(t) || is_u8(t) ||
         is_u16(t) || is_u32(t) || is_u64(t);
}
inline bool is_signed(const Type *t) {
  return is_i8(t) || is_i16(t) || is_i32(t) || is_i64(t);
}
...
protected:
  RefTypeBase(TypeTable &typetable, int tag, const Type *pointee, bool mut,
              uint64_t addr_space)
      : Type(typetable, tag, {pointee}), mut_(mut), addr_space_(addr_space) {}
...
protected:
  PtrType(TypeTable &typetable, int tag, const Type *pointee, bool mut,
          uint64_t addr_space)
      : RefTypeBase(typetable, tag, pointee, mut, addr_space) {}

  Stream &stream_ptr_type(Stream &, std::string prefix, uint64_t addr_space,
                          const Type *ref_type) const;
...
public:
  BorrowedPtrType(TypeTable &typetable, const Type *pointee, bool mut,
                  uint64_t addr_space)
...
protected:
  RefType(TypeTable &typetable, const Type *pointee, bool mut,
          uint64_t addr_space)
      : RefTypeBase(typetable, Tag_ref, pointee, mut, addr_space) {}

public:
  std::string prefix() const override {
    return is_mut() ? "lvalue of " : "reference of ";
  }
...
inline bool is_ptr(const Type *t) {
  return t->isa<PtrType>() ||
         (t->isa<RefType>() && t->as<RefType>()->pointee()->isa<PtrType>());
}
...
private:
  FnType(TypeTable &typetable, const Type *op) : Type(typetable, Tag_fn, {op}) {
    ++order_;
  }
...
private:
  Lambda(TypeTable &table, const Type *body, const char *name)
      : Type(table, Tag_lambda, {body}), name_(name) {}
...
private:
  Var(TypeTable &table, int depth) : Type(table, Tag_var, {}), depth_(depth) {
    monomorphic_ = false;
  }
...
private:
  TupleType(TypeTable &table, Types ops) : Type(table, Tag_tuple, ops) {}
...
private:
  StructType(TypeTable &table, const StructDecl *decl, size_t size)
      : Type(table, Tag_struct, mim::Vector<const Type *>(size)), decl_(decl) {
    nominal_ = true;
  }
...
public:
  ...
  void set(size_t i, const Type *type) const {
    return const_cast<StructType *>(this)->Type::set(i, type);
  }
...
private:
  EnumType(TypeTable &table, const EnumDecl *decl, size_t size)
      : Type(table, Tag_enum, mim::Vector<const Type *>(size)), decl_(decl) {
    nominal_ = true;
  }
...
public:
  ...
  void set(size_t i, const Type *type) const {
    return const_cast<EnumType *>(this)->Type::set(i, type);
  }
...
public:
  DefiniteArrayType(TypeTable &typetable, const Type *elem_type, uint64_t dim)
      : ArrayType(typetable, Tag_definite_array, elem_type), dim_(dim) {}

  uint64_t dim() const { return dim_; }
  uint32_t vhash() const override {
    return mim::hash_combine(Type::vhash(), dim());
  }
  bool equal(const Type *other) const override {
    return Type::equal(other) &&
           this->dim() == other->as<DefiniteArrayType>()->dim();
  }
...
public:
  SimdType(TypeTable &typetable, const Type *elem_type, uint64_t dim)
      : ArrayType(typetable, Tag_simd, elem_type), dim_(dim) {}
...
private:
  NoRetType(TypeTable &typetable) : Type(typetable, Tag_noret, {}) {}
...
private:
  UnknownType(TypeTable &typetable) : Type(typetable, Tag_unknown, {}) {
    known_ = false;
  }
...
private:
  TypeError(TypeTable &table) : Type(table, Tag_error, {}) {}
...
class TypeTable : public mim::TypeTableBase<Type> {
public:
  TypeTable();

  const Var *var(int depth) { return unify(new Var(*this, depth)); }
  const Type *app(const Type *callee, const Type *op);
  const Lambda *lambda(const Type *body, const char *name) {
    return unify(new Lambda(*this, body, name));
  }

  const TupleType *tuple_type(Types ops) {
    assert(ops.size() != 1);
    return unify(new TupleType(*this, ops));
  }
...
#include "impala/tokenlist.h"
  const DefiniteArrayType *definite_array_type(const Type *elem_type,
                                               uint64_t dim) {
    return unify(new DefiniteArrayType(*this, elem_type, dim));
  }
  const FnType *fn_type(const Type *op) { return unify(new FnType(*this, op)); }
  const FnType *fn_type(Types params) {
    return unify(new FnType(*this, params.size() == 1 ? params.front()
                                                      : tuple_type(params)));
  }
  ...
  const BorrowedPtrType *borrowed_ptr_type(const Type *pointee, bool mut,
                                           uint64_t addr_space) {
