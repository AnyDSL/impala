...
inline bool is_int(const Type* t) {
    return is_i8(t) || is_i16(t) || is_i32(t) || is_i64(t) || is_u8(t) || is_u16(t) || is_u32(t) || is_u64(t);
}
...
private:
    StructType(TypeTable& table, const StructDecl* decl, size_t size)
        : Type(table, Tag_struct, mim::Vector<const Type*>(size))
        , decl_(decl) {
        nominal_ = true;
    }
...
private:
    EnumType(TypeTable& table, const EnumDecl* decl, size_t size)
        : Type(table, Tag_enum, mim::Vector<const Type*>(size))
        , decl_(decl) {
        nominal_ = true;
    }
...
public:
    ...
    uint32_t vhash() const override { return mim::hash_combine(Type::vhash(), dim()); }
    bool equal(const Type* other) const override {
...
inline bool is_no_ret_or_type_error(const Type* t) { return t->isa<NoRetType>() || t->isa<TypeError>(); }

inline bool is_unit(const Type* t) { return t->isa<TupleType>() && t->num_ops() == 0; }
...
#include "impala/tokenlist.h"
    ...
    const FnType* fn_type(Types params) {
        return unify(new FnType(*this, params.size() == 1 ? params.front() : tuple_type(params)));
    }
