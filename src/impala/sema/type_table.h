...
protected:
    ...
    void set(size_t i, const TypeBase* type) {
        ...
        if (!is_nominal()) known_ &= type->is_known();
    }
...
const TypeBase<TypeTable>* TypeBase<TypeTable>::vreduce(int depth, const TypeBase* type, Type2Type& map) const {
    if (num_ops() > 0) {
        ...
        for (size_t i = 0, e = num_ops(); i != e; ++i) result[i] = op(i)->reduce(depth, type, map);
        return vrebuild(table(), result);
    }
    return this;
}

template<class TypeTable> const TypeBase<TypeTable>* TypeBase<TypeTable>::rebuild(TypeTable& to, Types ops) const {
    assert(num_ops() == ops.size());
    if (ops.empty() && &table() == &to) return this;
    return vrebuild(to, ops);
}

template<class TypeTable> uint32_t TypeBase<TypeTable>::vhash() const {
    if (is_nominal()) return mim::murmur3(uint32_t(gid()));

    uint32_t seed = mim::hash_begin(uint8_t(tag()));
    for (auto op : ops_) seed = mim::hash_combine(seed, uint32_t(op->gid()));
    return seed;
}

template<class TypeTable> bool TypeBase<TypeTable>::equal(const TypeBase* other) const {
    if (is_nominal()) return this == other;
...
template<class Type> const Type* TypeTableBase<Type>::unify_base(const Type* type) {
...
template<class Type> const Type* TypeTableBase<Type>::insert(const Type* type) {
