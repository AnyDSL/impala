...
public:
    ...
    const Var* infer(const ASTTypeParam* ast_type_param) {
        if (!ast_type_param->type()) ast_type_param->type_ = ast_type_param->infer(*this);
        return ast_type_param->type()->as<Var>();
    }

    const Type* infer(const ASTType* ast_type) { return constrain(ast_type, ast_type->infer(*this)); }
...
const Type* InferSema::reduce(const Lambda* lambda, ASTTypeArgs ast_type_args, std::vector<const Type*>& type_args) {
    ...
    if (ast_type_args.size() <= num) {
        ...
        while (type_args.size() < num) type_args.push_back(unknown_type());

        size_t i         = type_args.size();
        const Type* type = lambda;
        while (auto lambda = type->isa<Lambda>()) type = app(lambda, type_args[--i]);
...
const Type* InferSema::close(int num_lambdas, const Type* body) {
    auto result = body;
    while (num_lambdas-- != 0) result = lambda(result, "TODO");
...
void InferSema::assign(const Expr* dst, const Expr* src) {
    if (!dst->type()->isa<UnknownType>() && src->type()->is_known()) coerce(dst->type(), src);
}
...
const Type* InferSema::unify(const Type* dst, const Type* src) {
    ...
    if (dst->isa<UnknownType>() && src->isa<UnknownType>()) return unify_by_rank(dst_repr, src_repr)->type;
    ...
    if (dst->num_ops() == src->num_ops()) {
        ...
        if (auto dst_borrowed_ptr_type = dst->isa<BorrowedPtrType>()) {
            if (auto src_owned_ptr_type = src->isa<OwnedPtrType>()) {
                if (src_owned_ptr_type->addr_space() == dst_borrowed_ptr_type->addr_space())
                    return borrowed_ptr_type(unify(dst->op(0), src->op(0)), dst_borrowed_ptr_type->is_mut(),
        ...
        if (dst->tag() == src->tag()) {
            ...
            if (src->is_nominal() && src != dst) return infer_error(dst, src);

            Array<const Type*> op(dst->num_ops());
            for (size_t i = 0, e = op.size(); i != e; ++i) op[i] = unify(dst->op(i), src->op(i));
...
const Type* InferSema::find(const Type* type) { return find(representative(type))->type; }
...
const Var* ASTTypeParam::infer(InferSema& sema) const {
    for (auto&& bound : bounds()) sema.infer(bound.get());
    return sema.var(lambda_depth());
}

void ASTTypeParamList::infer_ast_type_params(InferSema& sema) const {
    for (auto&& ast_type_param : ast_type_params()) sema.infer(ast_type_param.get());
}
...
#define IMPALA_TYPE(itype, atype) \
    case TYPE_##itype: return sema.prim_type(PrimType_##itype);
...
const Type* IndefiniteArrayASTType::infer(InferSema& sema) const {
    return sema.indefinite_array_type(sema.infer(elem_ast_type()));
}
const Type* DefiniteArrayASTType::infer(InferSema& sema) const {
    return sema.definite_array_type(sema.infer(elem_ast_type()), dim());
}

const Type* TupleASTType::infer(InferSema& sema) const {
    Array<const Type*> types(num_ast_type_args());
    for (size_t i = 0, e = num_ast_type_args(); i != e; ++i) types[i] = sema.infer(ast_type_arg(i));
...
const Type* ASTTypeApp::infer(InferSema& sema) const {
    ...
    if (decl() && decl()->is_type_decl()) {
        if (auto ast_type_param = decl()->isa<ASTTypeParam>()) return sema.var(ast_type_param->lambda_depth_);
        auto type = sema.find_type(decl());
        if (auto lambda = type->isa<Lambda>()) return sema.reduce(lambda, ast_type_args(), type_args_);
        return type;
    }
...
const Type* Module::infer_head(InferSema&) const { /*TODO*/
    return nullptr;
}
const Type* ModuleDecl::infer_head(InferSema&) const { /*TODO*/
    return nullptr;
}
const Type* ExternBlock::infer_head(InferSema&) const { return nullptr; }
const Type* Typedef::infer_head(InferSema&) const { /*TODO*/
    return nullptr;
}
...
const Type* StructDecl::infer_head(InferSema& sema) const {
    ...
    for (size_t i = 0, e = num_field_decls(); i != e; ++i) struct_type->set(i, sema.infer(field_decl(i)));
    return struct_type;
}
...
const Type* EnumDecl::infer_head(InferSema& sema) const {
    ...
    for (size_t i = 0, e = num_option_decls(); i != e; ++i) enum_type->set(i, sema.infer(option_decl(i)));
    return enum_type;
}

const Type* StaticItem::infer_head(InferSema& sema) const {
    if (ast_type()) return sema.infer(ast_type());
    if (type_ == nullptr) type_ = sema.unknown_type();
    return nullptr;
}
...
const Type* FnDecl::infer_head(InferSema& sema) const {
    ...
    for (size_t i = 0, e = num_params(); i != e; ++i) param_types[i] = sema.infer(param(i));

    return sema.close(num_ast_type_params(), sema.fn_type(param_types));
}

const Type* TraitDecl::infer_head(InferSema&) const { /*TODO*/
    return nullptr;
}
const Type* ImplItem::infer_head(InferSema&) const { /*TODO*/
    return nullptr;
}
...
void Module::infer(InferSema& sema) const {
    for (auto&& item : items()) sema.infer_head(item.get());

    for (auto&& item : items()) sema.infer(item.get());
}

void ExternBlock::infer(InferSema& sema) const {
    for (auto&& fn_decl : fn_decls()) sema.infer(fn_decl.get());
}
...
const Type* OptionDecl::infer(InferSema& sema) const {
    if (num_args() != 0) {
        ...
        for (size_t i = 0, e = args().size(); i != e; ++i) params[i] = sema.infer(arg(i));
...
void FnDecl::infer(InferSema& sema) const {
    ...
    for (size_t i = 0; i != e; ++i) {
        ...
        if (type() && type()->isa<FnType>()) sema.constrain(param(i), fn_type()->param(i));
    }

    sema.constrain(this, sema.close(num_ast_type_params(), sema.fn_type(param_types)));

    if (body() != nullptr) {
        if (!sema.infer(body())->isa<NoRetType>()) sema.coerce(fn_type()->return_type(), body());
    }
}

void StaticItem::infer(InferSema& sema) const {
    if (ast_type()) sema.constrain(this, sema.infer(ast_type()));
    if (init()) sema.constrain(this, sema.rvalue(init()));
}
...
const Type* CharExpr::infer(InferSema& sema) const { return sema.type_u8(); }

const Type* StrExpr::infer(InferSema& sema) const { return sema.definite_array_type(sema.type_u8(), values_.size()); }
...
const Type* FnExpr::infer(InferSema& sema) const {
    ...
    if (num_params() == 0 && body_type->isa<NoRetType>()) return sema.fn_type(sema.unit());
...
const Type* PrefixExpr::infer(InferSema& sema) const {
    switch (tag()) {
        ...
        case MUT: {
            auto type = sema.infer(rhs());
            if (auto ref = type->isa<RefType>()) return sema.borrowed_ptr_type(ref->pointee(), true, ref->addr_space());
            // The type might turn out to be a ref type later on
            if (type->isa<UnknownType>()) return sema.find_type(this);
            return sema.borrowed_ptr_type(type, true, 0);
        }
        case TILDE: return sema.owned_ptr_type(sema.rvalue(rhs()), 0);
        case MUL: {
        ...
        case INC:
        case DEC: return unpack_ref_type(sema.infer(rhs()));
        case ADD:
        case SUB:
        case NOT:
        case HLT: return sema.rvalue(rhs());
        case KNOWN: sema.infer(rhs()); return sema.type_bool();
        case RUNRUN: return sema.infer(rhs());
        case OR:
        case OROR:
...
const Type* PostfixExpr::infer(InferSema& sema) const { return unpack_ref_type(sema.infer(lhs())); }
...
const Type* RValueExpr::infer(InferSema& sema) const {
    ...
    if (auto ref_type = src_type->isa<RefType>()) return ref_type->pointee();
    // function calls never result in references
    if (auto map = src()->isa<MapExpr>()) {
        if (map->lhs()->type()->isa<FnType>()) return src_type;
    }
...
const Type* DefiniteArrayExpr::infer(InferSema& sema) const {
    ...
    for (auto&& arg : args()) sema.rvalue(arg.get());

    for (auto&& arg : args()) expected_elem_type = sema.coerce(expected_elem_type, arg.get());
...
const Type* TupleExpr::infer(InferSema& sema) const {
    ...
    for (size_t i = 0, e = types.size(); i != e; ++i) types[i] = sema.rvalue(arg(i));
...
const Type* StructExpr::infer(InferSema& sema) const {
    ...
    for (size_t i = 0, e = num_elems(); i != e; ++i) sema.rvalue(elem(i)->expr());
...
const Type* InferSema::infer_call(const Expr* lhs, Span<const Expr*> args, const Type* call_type) {
    ...
    if (args.size() == fn_type->num_params()) {
        ...
        for (size_t i = 0, e = args.size(); i != e; ++i) types[i] = coerce(fn_type->param(i), args[i]);
    ...
    if (args.size() + 1 == fn_type->num_params()) {
        ...
        if (auto fn_type = result->isa<FnType>()) return fn_type->return_type();
        return call_type;
    }
    ...
    if (auto type = sema.rvalue(lhs())) {
        if (auto lambda = type->isa<Lambda>()) {
            ...
            if (type_args_.size() < num) {
                ...
                while (num_type_args() < num) type_args_.push_back(sema.unknown_type());
            }
...
const Type* MapExpr::infer(InferSema& sema) const {
    ...
    if (ltype->isa<UnknownType>()) return sema.find_type(this);

    if (auto array = ltype->isa<ArrayType>()) return sema.wrap_ref(ref, array->elem_type());
    ...
    if (ref && (ltype->isa<Lambda>() || ltype->isa<FnType>())) ltype = sema.rvalue(lhs());

    if (ltype->isa<Lambda>()) {
        if (!lhs()->isa<TypeAppExpr>()) TypeAppExpr::create(lhs());
        ltype = sema.infer(lhs());
    }

    if (ltype->isa<FnType>()) return sema.infer_call(lhs(), args(), sema.find_type(this));
...
const Type* BlockExpr::infer(InferSema& sema) const {
    for (auto&& stmt : stmts())
        if (auto item_stmt = stmt->isa<ItemStmt>()) sema.infer_head(item_stmt->item());

    for (auto&& stmt : stmts()) sema.infer(stmt.get());
...
const Type* ForExpr::infer(InferSema& sema) const {
    ...
    if (auto map = forexpr->isa<MapExpr>()) {
        ...
        for (size_t i = 0, e = map->num_args(); i != e; ++i) sema.rvalue(map->arg(i));

        if (auto fn_for = ltype->isa<FnType>()) {
            if (fn_for->num_params() != 0) {
                if (auto fn_ret = fn_for->last_param()->isa<FnType>())
                    sema.constrain(break_decl_.get(),
                                   fn_ret); // inherit the type for break
            }
...
const Type* RevDiffExpr::infer(InferSema& sema) const {
    if (auto fn_type = sema.infer(expr())->isa<FnType>()) return fn_type->rev_diffed_type();
    return sema.type_error();
}
...
const Type* IdPtrn::infer(InferSema& sema) const { return sema.infer(local()); }

const Type* EnumPtrn::infer(InferSema& sema) const {
    auto ret_type = sema.find_type(this);
    if (num_args() > 0) {
        Array<const Type*> params(num_args() + 1);
        for (size_t i = 0, e = num_args(); i != e; ++i) params[i] = sema.infer(arg(i));
...
const Type* LiteralPtrn::infer(InferSema& sema) const { return sema.infer(literal()); }

const Type* CharPtrn::infer(InferSema& sema) const { return sema.infer(chr()); }
