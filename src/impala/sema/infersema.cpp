...
public:
  ...
  const Type *reduce(const Lambda *lambda, ASTTypeArgs ast_type_args,
                     std::vector<const Type *> &type_args);
  void fill_type_args(std::vector<const Type *> &type_args,
                      const ASTTypes &ast_type_args);
  ...
  const Type *&constrain(const Typeable *t, const Type *u, const Type *v) {
    return constrain(constrain(t, u), v);
  }
  const Type *&constrain(const Typeable *t, const Type *u) {
    return constrain(t->type_, u);
  }

  /// obeys subtyping
  const Type *coerce(const Type *dst, const Expr *src);
  const Type *coerce(const Typeable *dst, const Expr *src) {
    return dst->type_ = coerce(dst->type_, src);
  }
  ...
  const Type *infer(const FieldDecl *f) {
    return constrain(f, f->infer(*this));
  }
  const Type *infer(const OptionDecl *o) {
    return constrain(o, o->infer(*this));
  }
  void infer(const Item *n) { n->infer(*this); }
  const Type *infer_head(const Item *n) {
    return (n->type_ == nullptr || n->type_->isa<UnknownType>())
               ? n->type_ = n->infer_head(*this)
               : n->type_;
  }
  void infer(const Stmt *n) { n->infer(*this); }
  const Type *infer(const Expr *expr) {
    return constrain(expr, expr->infer(*this));
  }
  const Type *infer(const Expr *expr, const Type *t) {
    return constrain(expr, expr->infer(*this), t);
  }
  const Type *infer(const Path *path) {
    return constrain(path, path->infer(*this));
  }
  const Type *infer(const Path *path, const Type *t) {
    return constrain(path, path->infer(*this), t);
  }
  ...
  const Type *infer_call(const Expr *lhs, Span<const Expr *> args,
                         const Type *call_type);
  const Type *infer_call(const Expr *lhs, const Exprs &args,
                         const Type *call_type) {
  ...
  const Type *rvalue(const Expr *expr) {
    ...
    if (type->isa<RefType>() ||
        (type->isa<UnknownType>() && !expr->isa<RValueExpr>())) {
      todo_ = true;
...
private:
  ...
  struct Representative {
    Representative() {}
    Representative(const Type *type) : parent(this), type(type) {}
  ...
  friend void type_inference(std::unique_ptr<TypeTable> &typetable,
                             const Module *);
};
...
const Type *InferSema::reduce(const Lambda *lambda, ASTTypeArgs ast_type_args,
                              std::vector<const Type *> &type_args) {
...
void InferSema::fill_type_args(std::vector<const Type *> &type_args,
                               const ASTTypes &ast_type_args) {
...
const Type *InferSema::coerce(const Type *dst, const Expr *src) {
  ...
  if (auto borrowed_ptr_type = dst->isa<BorrowedPtrType>()) {
    if (!borrowed_ptr_type->is_mut() && !src->type()->isa<UnknownType>() &&
        !src->type()->isa<PtrType>())
      infer(src = PrefixExpr::create_addrof(src));
  }

  // insert implicit cast for subtyping
  if (dst->is_known() && src->type()->is_known() &&
      is_strict_subtype(dst, src->type()))
...
const Type *InferSema::unify(const Type *dst, const Type *src) {
  ...
  if (src->isa<TupleType>() && src->num_ops() == 1)
    src = src->op(0);
  if (dst->isa<TupleType>() && dst->num_ops() == 1)
    dst = dst->op(0);

  if (dst->isa<UnknownType>() && src->isa<UnknownType>())
    return unify_by_rank(dst_repr, src_repr)->type;
  if (dst->isa<UnknownType>())
    return unify(src_repr, dst_repr)->type;
  if (src->isa<UnknownType>())
    return unify(dst_repr, src_repr)->type;

  if (dst == src && dst->is_known())
    return dst;
  if (dst->isa<TypeError>() || dst->isa<InferError>())
    return dst; // propagate errors
  if (src->isa<TypeError>() || src->isa<InferError>())
    return src; // dito
  ...
  if (dst->num_ops() == src->num_ops()) {
    ...
    if (auto dst_borrowed_ptr_type = dst->isa<BorrowedPtrType>()) {
      if (auto src_owned_ptr_type = src->isa<OwnedPtrType>()) {
        if (src_owned_ptr_type->addr_space() ==
...
auto InferSema::representative(const Type *type) -> Representative * {
  ...
  if (i == representatives_.end()) {
    auto p =
        representatives_.emplace(type, std::make_unique<Representative>(type));
...
auto InferSema::unify(Representative *x, Representative *y)
    -> Representative * {
...
auto InferSema::unify_by_rank(Representative *x, Representative *y)
    -> Representative * {
  assert(x->is_root() && y->is_root());
...
void type_inference(std::unique_ptr<TypeTable> &typetable,
                    const Module *module) {
...
const Type *Path::infer(InferSema &sema) const {
  ...
  for (size_t i = 1, e = num_elems(); i != e; ++i) {
    ...
    if (auto enum_type = last_type->isa<EnumType>()) {
      ...
      auto option_type =
          option_decl ? sema.find_type(*option_decl) : sema.type_error();
...
const Type *ErrorASTType::infer(InferSema &sema) const {
  return sema.type_error();
}
...
#define IMPALA_TYPE(itype, atype)                                              \
  case TYPE_##itype:                                                           \
    return sema.prim_type(PrimType_##itype);
...
const Type *PtrASTType::infer(InferSema &sema) const {
  ...
  switch (tag()) {
  case Borrowed:
    return sema.borrowed_ptr_type(pointee, false, addr_space());
  case Mut:
    return sema.borrowed_ptr_type(pointee, true, addr_space());
  case Owned:
    return sema.owned_ptr_type(pointee, addr_space());
  }
  fe::unreachable();
}
...
const Type *Typeof::infer(InferSema &sema) const {
  return unpack_ref_type(sema.infer(expr()));
}
...
const Type *FieldDecl::infer(InferSema &sema) const {
  return sema.infer(ast_type());
}
...
void FnDecl::infer(InferSema &sema) const {
  ...
  sema.constrain(this,
                 sema.close(num_ast_type_params(), sema.fn_type(param_types)));
...
const Type *LiteralExpr::infer(InferSema &sema) const {
  return sema.prim_type(literal2type());
}
...
const Type *FnExpr::infer(InferSema &sema) const {
  ...
  if (body_type->isa<NoRetType>() || body_type->isa<UnknownType>())
    ...
    if (num_params() == 0)
      return sema.fn_type(sema.fn_type(sema.type_error()));

    param_types.back() =
        sema.constrain(params().back().get(), sema.fn_type(body_type));
    return sema.fn_type(param_types);
  }
}
...
const Type *PrefixExpr::infer(InferSema &sema) const {
  ...
  case MUL: {
    ...
    if (auto ptr_type = type->isa<PtrType>())
      return sema.ref_type(ptr_type->pointee(), ptr_type->is_mut(),
                           ptr_type->addr_space());
    else
      return sema.find_type(this);
  }
...
const Type *StructExpr::infer(InferSema &sema) const {
  ...
  if (auto struct_type = type->isa<StructType>()) {
    for (size_t i = 0, e = num_elems(); i != e; ++i) {
      elem(i)->field_decl_ = struct_type->struct_decl()
                                 ->field_decl(elem(i)->symbol())
                                 .value_or(nullptr);
      if (elem(i)->field_decl() != nullptr)
        sema.coerce(struct_type->op(elem(i)->field_decl()->index()),
                    elem(i)->expr());
    }
  }
...
const Type *InferSema::infer_call(const Expr *lhs, Span<const Expr *> args,
                                  const Type *call_type) {
...
const Type *IfExpr::infer(InferSema &sema) const {
  ...
  if (then_type->isa<NoRetType>() || then_type->isa<UnknownType>())
    return else_type;
  if (else_type->isa<NoRetType>() || else_type->isa<UnknownType>())
    return then_type;
...
const Type *MatchExpr::infer(InferSema &sema) const {
  ...
  for (size_t i = 0, e = num_arms(); i != e; ++i) {
    ...
    if (!arm_type->isa<NoRetType>())
      match_type =
          match_type ? sema.constrain(arm(i)->expr(), match_type) : arm_type;
  }
...
const Type *TuplePtrn::infer(InferSema &sema) const {
  auto types = Array<const Type *>(
      num_elems(), [&](auto i) { return sema.infer(this->elem(i)); });
  return sema.tuple_type(types);
}
...
void AsmStmt::infer(InferSema &sema) const {
  for (auto &&output : outputs())
    sema.infer(output->expr());
  for (auto &&input : inputs())
    sema.rvalue(input->expr());
}
